...
class GapWrapper(Protocol):
    data: pd.DataFrame

    def get_max_gap(self) -> "GapWrapper": ...

    def get_gap_start_end(self) -> pd.DataFrame: ...


class GapFiller(Protocol):
    def fill(self, data: pd.DataFrame, gaps: GapWrapper, resample_rule="min", **kwargs) -> pd.DataFrame: ...


class LinearGapFiller:

    @classmethod
    def fill(cls, data: pd.DataFrame, gaps: GapWrapper, resample_rule="min", **kwargs) -> pd.DataFrame:
        out_data = data.copy()
        ...
        gap_start_times = gaps.data["start"]

        gap_end_times = gaps.data["end"]
